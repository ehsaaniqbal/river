import { RANK_VALUE, evaluateBestHand, type ActionType, type Card, type GameState, type Player, type Position } from '@river/engine';
...
function isAdvancedMixedHand(position: Position, notation: string): boolean {
  ...
  return ADVANCED_MIXED_HANDS[position]?.has(notation) ?? false;
}
...
function shouldPlayPreflop(botPlayer: Player): boolean {
  ...
  const range = RANGES[botPlayer.botDifficulty ?? 'BEGINNER'][botPlayer.position] ?? new Set<string>();
