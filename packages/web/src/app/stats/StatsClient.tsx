...
import type { PlayerStats } from '@river/engine';
...
export default function StatsPage() {
  const localStats = useGameStore((state) => state.playerStats);
  ...
  const stats = serverUser ? playerStatsFromUser(serverUser, localStats) : localStats;
  const biggestPotWon = serverUser?.biggestPotWon ?? localBiggestPotWon(handHistory);
  const profitValues = useMemo(() => (
    serverUser
      ? multiplayerProfitValues(serverHands, serverUser.id)
      : localProfitValues(handHistory)
  ), [handHistory, serverHands, serverUser]);
    ...
    <main className="min-h-screen bg-[#06110d] text-[var(--text-primary)]">
      ...
      <div className="mx-auto max-w-7xl px-5 pb-8 pt-28">
        ...
        <section className="mb-6 grid gap-3 md:grid-cols-2 lg:grid-cols-5">
          ...
          <StatCard title="Biggest Pot" value={biggestPotWon} />
...
function playerStatsFromUser(user: SessionUser, localStats: PlayerStats): PlayerStats {
  return {
    ...
    vpip: user.vpip,
    pfr: user.pfr,
    pokerIQ: localStats.pokerIQ,
    sessionActions: localStats.sessionActions,
  };
}

function localProfitValues(handHistory: ReturnType<typeof useGameStore.getState>['handHistory']): number[] {
  return handHistory
    .slice()
    .reverse()
...
function multiplayerProfitValues(hands: HandHistoryRecord[], userId: string): number[] {
  return hands
    .slice()
    .reverse()
    .reduce<number[]>((rows, hand) => {
      const previous = rows.at(-1) ?? 0;
      const hero = hand.players.find((player) => player.id === userId);
      rows.push(previous + (hero ? hero.stackAfter - hero.stackBefore : 0));
      return rows;
    }, []);
}

function localBiggestPotWon(handHistory: ReturnType<typeof useGameStore.getState>['handHistory']): number {
  return handHistory.reduce((biggest, hand) => {
    const heroWin = hand.winners.filter((winner) => winner.playerId === 'hero').reduce((sum, winner) => sum + winner.amount, 0);
    return Math.max(biggest, heroWin);
  }, 0);
}
